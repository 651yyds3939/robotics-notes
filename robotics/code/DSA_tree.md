...
* **📝 对应冲刺题目：**
* **94. 二叉树的中序遍历（简单）** $\rightarrow$ *你的第一题，纯粹抄写和理解网课里的递归模板。*
...
这个阶段你将接触到机器人在地图搜索里最常用的算法——**BFS（广度优先搜索）**。
...
* **199. 二叉树的右视图（中等）** $\rightarrow$ *层序遍历的变种，相当于你站在右边看这棵树，其实就是层序遍历时，每一层只打印最后一个节点。*
...
* **101. 对称二叉树（简单）** $\rightarrow$ *双树同步递归，考察你能不能同时操纵两个指针。*
...
这 15 道题里，几乎 **80% 的代码都是用递归写出来的**。写递归时如果觉得脑子绕进去了，记住一个秘诀：**不要试图用大脑去模拟每一层的调用栈**。你只需要想好两件事：

1. **Base Case（什么时候停下来）：** 通常是 `if (root == nullptr) return ...;`
2. **单层逻辑（当前节点和左右儿子什么关系）：** 做好当前节点的事，剩下的放心抛给 `dfs(root->left)` 和 `dfs(root->right)`。

你打算今天先把第一阶段的视频看完，然后把第一道题《94. 二叉树的中序遍历》给撕掉吗？写完可以随时把代码发给我帮你 review。