...
> **大白话**：SLAM 就是"边走边画地图，边看地图边确定自己在哪"。你进入一个黑房间，摸墙走，同时脑子里画房间的形状——这就是 SLAM。问题在于，你画的墙可能有误差，你走的步数也有误差，两个误差互相放大，你就迷路了。SLAM 算法就是解决"两个误差互相掐架"的问题。
...
```text
IMU (200Hz) ──→ 预积分预测位姿 ──→ 去畸变点云
                                      ↓
LiDAR (10Hz) ──→ 特征提取 ──→ 紧耦合优化 ──→ 更新位姿 + 地图
```
